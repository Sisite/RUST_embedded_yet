fn main() {
    ...
    decode(&mut seed, &wordarr, &mut bytearr, 0);
    for x in bytearr.iter() {
        print!("{}", *x as char);
    }
}

fn decode(mut seed: &mut u32, wordarr: &[u32], bytearr: &mut [u8], mut recdepth: usize) -> u32 {
    let m: u32;
    let r: u32;
    let y: u32;
    let x: u32 = !codgen(seed);
